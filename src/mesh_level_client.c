...
extern void adv_send_cmd_lightness_change (uint8_t flag, uint16_t transition_time);
...
void mesh_level_client_set_delta(wiced_bt_mesh_event_t *p_event, uint8_t *p_data, uint32_t length)
{
    ...
    STREAM_TO_UINT32(set_data.transition_time, p_data);
    STREAM_TO_UINT16(set_data.delay, p_data);
    WICED_LOG_DEBUG(" Set delta: dst = 0x%x, elem = %d\n", p_event->dst, p_event->element_idx);
...
void mesh_level_reduce(int8_t delta,uint16_t tran_time)
{
    uint8_t pack_load[10];
    uint8_t length;
    ...
    WICED_LOG_DEBUG(" VENDOR_DELTABRIGHTNESSLESS_CMD: %d\r\n",delta);
    length = 0;
    pack_load[length++] = delta;
    pack_load[length++] = tran_time;

    mesh_vendor_send_cmd(VENDOR_DELTABRIGHTNESS_CMD,pack_load,length);
    ...
    p_event = wiced_bt_mesh_create_event(element_idx, MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_GENERIC_ONOFF_CLNT, dst, app_key_idx);
    if (p_event == NULL)
    {
        //WICED_LOG_DEBUG(" Send user adv Start\r\n");
        WICED_LOG_DEBUG("%s: Failed to create mesh event\n", __func__);
        //adv_send_cmd_lightness_change(0,200);
        return;
    }
    p_event->reply = 1;   // Request reply

    length = 0;
    UINT32_TO_STREAM(p_data, (-65535/5));       // onoff
    level = (-65535/5);
    length += 4;
    UINT8_TO_STREAM(p_data, WICED_BT_MESH_LEVEL_DELTA_START);          // continuation
    length += 1;
    UINT32_TO_STREAM(p_data, (uint32_t) 0);        // transition_time
    t_time = 200;
    length += 4;
    UINT16_TO_STREAM(p_data, (uint16_t) 0);          // delay
    length += 2;

    WICED_LOG_DEBUG(" delta:%d, t_time:%d\r\n",level,t_time);
    mesh_level_client_set_delta(p_event, data, length);
    */
}

void mesh_level_plus(int8_t delta,uint16_t tran_time)
{
    ...
    WICED_LOG_DEBUG(" VENDOR_DELTABRIGHTNESSPLUS_CMD: %d\r\n",delta);
    length = 0;
    pack_load[length++] = delta;
    pack_load[length++] = tran_time;
    mesh_vendor_send_cmd(VENDOR_DELTABRIGHTNESS_CMD,pack_load,length);
    ...
    {
        //WICED_LOG_DEBUG(" Send user adv Start\r\n");
        WICED_LOG_DEBUG("%s: Failed to create mesh event %04x %04x\n", __func__, dst, app_key_idx);
        //adv_send_cmd_lightness_change(1,200);
        return;
    }
    p_event->reply = 1;   // Request reply

    length = 0;
    UINT32_TO_STREAM(p_data, (uint32_t) (65535/5));          // onoff
    level = (65535/5);
    length += 4;
    UINT8_TO_STREAM(p_data, WICED_BT_MESH_LEVEL_DELTA_START);          // continuation
    length += 1;
