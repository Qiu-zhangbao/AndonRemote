...
#define MESH_APP_CONFIG_TIMEOUT     60    // configuration timeout is 60 seconds
...
wiced_bt_mesh_core_received_msg_handler_t mesh_app_raw_access_layer_mode_msg_handler_ptr = NULL;
wiced_bt_mesh_core_received_msg_handler_t mesh_app_raw_access_layer_mode_app_handler_ptr = NULL;
uint16_t mesh_app_raw_access_layer_clnt_addr = 0;
uint16_t mesh_app_raw_access_layer_clnt_netkey_idx = 0;
uint8_t mesh_app_raw_access_layer_clnt_devkey[16];
wiced_bt_mesh_event_t* mesh_app_send_raw_access_layer_pdu_p_event = NULL;

wiced_bool_t mesh_app_raw_access_layer_mode_msg_handler(wiced_bt_mesh_event_t* p_event, const uint8_t* params, uint16_t params_len);
#endif
...
// Local functions       
       ...
       void                     mesh_interrupt_handler(void* user_data, uint8_t value);
...
static void                     mesh_application_provision_server_callback(uint16_t event, void *p_data);
static void                     mesh_application_iv(uint32_t iv_index, uint8_t iv_update_flag);


static void mesh_app_config_timer(void);
...
extern uint8_t mesh_app_provision_status;
...
// Mesh node interrupt handler
mesh_app_interrupt_handler_t mesh_app_interrupt_cb = NULL;
wiced_bt_mesh_provision_server_callback_t *mesh_app_provision_handler = NULL;
//wiced_bt_ble_userscan_result_cback_t *adv_manufacturer_handler = NULL;

// Mesh application post initialization handler
wiced_bt_mesh_app_post_init_t mesh_app_post_init = NULL;

// Mesh node LPN/Friend/Sleep state change callback functions
mesh_app_frnd_friendship_cb_t mesh_app_frnd_friendship_cb = NULL;
mesh_app_lpn_friendship_cb_t  mesh_app_lpn_friendship_cb  = NULL;

// Mesh provisioner node network open function
//mesh_app_provisioner_network_open_t mesh_app_provisioner_network_open_func = NULL;
...
#include "wiced_rtos.h"
...
#else
        //wiced_platform_register_button_callback(WICED_PLATFORM_BUTTON_1, mesh_interrupt_handler, NULL, GPIO_EN_INT_FALLING_EDGE);
        //wiced_platform_register_button_callback(WICED_PLATFORM_BUTTON_2, mesh_interrupt_handler, NULL, GPIO_EN_INT_BOTH_EDGE);
        //wiced_platform_register_button_callback(WICED_PLATFORM_BUTTON_3, mesh_interrupt_handler, NULL, GPIO_EN_INT_BOTH_EDGE);
        wiced_platform_register_button_callback(WICED_PLATFORM_BUTTON_4, mesh_interrupt_handler, NULL, GPIO_EN_INT_FALLING_EDGE);
        wiced_platform_register_button_callback(WICED_PLATFORM_BUTTON_5, mesh_interrupt_handler, NULL, GPIO_EN_INT_BOTH_EDGE);
    ...
    {
        LOG_DEBUG( "mesh cold Start \n");
        boot_mode = 0;
    }
    else
    {
        LOG_DEBUG( "mesh warm Start \n");
        boot_mode = 1;
//        pin_status = wiced_hal_gpio_get_pin_input_status(WICED_P14);
//        LOG_DEBUG( "WICED_P14 pin_status: %d \n", pin_status);
//        pin_status = wiced_hal_gpio_get_pin_input_status(WICED_P26);
//        LOG_DEBUG( "WICED_P26 pin_status: %d\n", pin_status);
//        pin_status = wiced_hal_gpio_get_pin_input_status(WICED_P27);
//        LOG_DEBUG( "WICED_P27 pin_status: %d \n", pin_status);
    }
}
...
wiced_result_t mesh_management_cback(wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data)
{
    ...
    // LOG_DEBUG("mesh_management_cback: %x\n", event);
    //test_aes();
    ...
    case BTM_ENABLED_EVT:
        //WICED_BT_TRACE("%s %d: ## mesh_application_init free_bytes:%d ##\n",__func__,__LINE__, wiced_memory_get_free_bytes());
#ifdef _DEB_DELAY_START_SEC
    ...
    case BTM_BLE_ADVERT_STATE_CHANGED_EVT:
        ...
        LOG_DEBUG("Advertisement State Changed:%d\n", *p_mode);
...
#ifndef NODE_UUID_HAS_MAGIC_NUM
...
void mesh_application_factory_reset(void)
{
    ...
    // WICED_BT_TRACE_CY("mesh_application_factory_reset: *************************\n");

    // uncomment following lines to delete UUID from the NV. This will change UUID on factory reset.
    // wiced_result_t result;
    // uint8_t deviceUUID[16] = {0};
    mesh_nvram_access(WICED_TRUE, NVRAM_ID_LOCAL_IV, NULL, 0, &result);
    //mesh_nvram_access(WICED_TRUE, NVRAM_ID_LOCAL_PROVISION, NULL, 0, &result);
    // mesh_nvram_access(WICED_TRUE, NVRAM_ID_LOCAL_UUID, deviceUUID, 0, &result);
    LOG_DEBUG("mesh_application_factory_reset: *************************\n");
...
{
    extern void appSetConnAdvDisable(void);
    LOG_DEBUG("mesh_ota_firmware_upgrade_status_callback: status:%d\n", status);
    ...
    else if(OTA_FW_UPGRADE_STATUS_COMPLETED == status){
        appSetConnAdvDisable();
    }
}

extern wiced_platform_button_config_t platform_button[];
extern void mesh_onoff_btn_event_handler(int btn_down_time);
...
void mesh_application_init(void)
{
    ...
    WICED_BT_TRACE("%s %d: ## mesh_application_init free_bytes:%d ##\n", __func__,__LINE__, wiced_memory_get_free_bytes());
    //wiced_bt_btn_init("Zhw Switch LPN", mesh_onoff_btn_event_handler);
#ifndef WICEDX_LINUX
...
//#endif
    ...
    */
    ...
    wiced_bt_mesh_core_proxy_on_demand_advert_to = 0;
    // wiced_bt_mesh_core_provisioning_srv_adv_interval = 160;
    ...
    {
        WICED_BT_TRACE("core_init failed %d\n", result);
    ...
    {
        WICED_BT_TRACE("ota_fw_upgrade_init failed\n");
    }
#endif
    ...
    {
        wiced_bt_mesh_app_provision_server_init(pb_priv_key, mesh_application_provision_server_callback);
    }
    ...
    if (result == WICED_BT_SUCCESS && len == 1 && provision_status == MESH_APP_PROVISION_STATUS_PROVISIONING)
    {
        // Looks like the board reboot or stucked in the middle of provisioning/configuring.
        // Do a factory reset to clean up everything.
        provision_status = MESH_APP_PROVISION_STATUS_PRORESTART;
        mesh_nvram_access(WICED_TRUE, NVRAM_ID_LOCAL_PROVISION, &provision_status, sizeof(uint8_t), &result);
        mesh_application_factory_reset();
    }
...
static void mesh_adv_report(wiced_bt_ble_scan_results_t *p_scan_result, uint8_t *p_adv_data)
{
    ...
    else if (p_scan_result->ble_evt_type == BTM_BLE_EVT_NON_CONNECTABLE_ADVERTISEMENT)
    {
        ...
        if( (adv_manufacturer_handler != NULL) && (p_adv_data[1] == BTM_BLE_ADVERT_TYPE_MANUFACTURER) 
            && (p_adv_data[2] == 0x04) && (p_adv_data[3] == 0x08) )
        {
            if((*adv_manufacturer_handler)(p_scan_result,p_adv_data))
            {
                return;
            }
        }
...
void mesh_interrupt_handler(void* user_data, uint8_t pin)
{
    ...
    //LOG_DEBUG("interrupt_handler: pin:%d value:%d\n", pin, value);
    //LOG_DEBUG("[App] ***** mesh_interrupt_handler \r\n");
    //LOG_DEBUG("[App] mesh_app_interrupt_cb = 0x%X \r\n",mesh_app_interrupt_cb);
    if (mesh_app_interrupt_cb != NULL)
    {
        //if (mesh_app_node_is_provisioned())
        {
            (*mesh_app_interrupt_cb)(user_data, pin, mesh_app_node_is_provisioned());
        }
    }
    else if (value != 0)
    {
...
#endif
#endif
    //LOG_DEBUG("mesh_nvram_access: inx:%x write:%d result:%x len:%d/%d\n", inx, write, *p_result, len, len_res);
    return len_res;
}
...
{
    uint8_t     ret = 0;
    LOG_DEBUG("mesh_fault_test: element:%d test_id:%d company_id:%x\n", element, test_id, company_id);
...
wiced_bool_t mesh_publication_callback(uint8_t elem_idx, uint16_t company_id, uint16_t model_id, uint16_t period)
{
    ...
    LOG_DEBUG("mesh_publication_callback: idx:%d company_id:%x model_id:%x period:%04x\n", elem_idx, company_id, model_id, period);
...
wiced_bt_mesh_core_received_msg_handler_t get_msg_handler_callback(uint16_t company_id, uint16_t opcode, uint16_t *p_model_id, uint8_t *p_rpl_delay)
{
    ...
    else
    {
        ...
        for (idx_elem = 0; idx_elem < mesh_config.elements_num; idx_elem++)
        {
            ...
            {
                if (mesh_config.elements[idx_elem].models[idx_model].company_id != company_id)
                    continue;
    ...
    WICED_BT_TRACE("CID:%x opcode:%x mode_id:%x\n", company_id, opcode,*p_model_id);
...
#if defined HCI_CONTROL
    // If we are in the Raw Access Layer Mode then use special handler
    if (mesh_app_raw_access_layer_mode_msg_handler_ptr != NULL)
    {
        //WICED_BT_TRACE("raw_access_layer_mode\n");
        mesh_app_raw_access_layer_mode_app_handler_ptr = p_message_handler;
        p_message_handler = mesh_app_raw_access_layer_mode_msg_handler;
    }
#endif
...
void mesh_start_stop_scan_callback(wiced_bool_t start, wiced_bool_t is_scan_active)
{
    ...
    // LOG_DEBUG("scan callback: start:%d active:%d local_addr:%x pb_gatt:%d\n", start, is_scan_active, local_addr, pb_gatt_in_progress);
    ...
    {
        // WICED_BT_TRACE_CY("observe stop\n");
    ...
    // WICED_BT_TRACE_CY("observe start active:%d\n", active);
...
void mesh_state_changed_cb(wiced_bt_mesh_core_state_type_t type, wiced_bt_mesh_core_state_t *p_state)
{
    ...
    wiced_result_t          result;
    wiced_bool_t            provisioned_over_gatt = WICED_FALSE;
    const char              *str = "state_changed: ";
    ...
    case WICED_BT_MESH_CORE_STATE_TYPE_SEQ:
        ...
        //LOG_DEBUG("mesh_state_changed_cb: addr:%x seq:%x previous_iv_idx:%d\n", p_state->seq.addr, p_state->seq.seq, p_state->seq.previous_iv_idx);
#ifdef HCI_CONTROL
    ...
    case WICED_BT_MESH_CORE_STATE_NODE_STATE:
        LOG_DEBUG("%sauthenticated:%d provisioned:%d proxy_on:%d pb_adv:%d\n", str, node_authenticated, p_state->node_state.provisioned, p_state->node_state.proxy_on, p_state->node_state.pb_adv);
        ...
        if (mesh_app_gatt_is_connected() && !node_authenticated && p_state->node_state.provisioned)
        {
            ...
            provisioned_over_gatt = WICED_TRUE;
        }
...
#ifndef MESH_HOMEKIT_COMBO_APP
        mesh_app_gatt_db_init(node_authenticated);
        if (provisioned_over_gatt)
        {
            mesh_app_gatt_send_service_changed_indication();
        }
#else // MESH_HOMEKIT_COMBO_APP
    ...
    case WICED_BT_MESH_CORE_STATE_LPN_FRIENDSHIP:
        LOG_DEBUG("%sLPN established:%d addr:%x/%x\n", str, p_state->lpn.established, p_state->lpn.addr, wiced_bt_mesh_core_lpn_get_friend_addr());
#ifdef _DEB_LPN_RSP_TIME
    ...
    case WICED_BT_MESH_CORE_STATE_LPN_SLEEP:
        LOG_DEBUG("%sLPN_SLEEP timeout:%d\n", str, p_state->lpn_sleep);
...
//         wiced_bt_mesh_core_shutdown();
        ...
        // Johnson 2020.08.01
        // ================================================
        extern void mesh_app_lpn_start_idle_timer(uint32_t duration);
        mesh_app_lpn_start_idle_timer(p_state->lpn_sleep);        
        // ==============================================
        
        break;

    case WICED_BT_MESH_CORE_STATE_FRND_FRIENDSHIP:
        LOG_DEBUG("%sFRIEND established:%d addr:%x\n", str, p_state->frnd.established, p_state->frnd.addr);
        if (mesh_app_frnd_friendship_cb != NULL)
        {
            (*mesh_app_frnd_friendship_cb)(p_state->frnd.established, p_state->frnd.addr);
        }
        break;
        
    case WICED_BT_MESH_CORE_STATE_APP_KEY_ADD:
        LOG_DEBUG("%sAPP_KEY_ADD net_key_idx:%x app_key_idx:%x\n", str, p_state->app_key.net_key_index, p_state->app_key.index);
#ifdef ENABLE_SELF_CONFIG_MODEL
        wiced_bt_mesh_self_config_key_bind(p_state->app_key.index);
#else
        wiced_bt_mesh_app_set_configured();
#endif
        break;
        /*
    case WICED_BT_MESH_CORE_STATE_MODEL_APP_BIND:
        LOG_DEBUG("%sBIND model:%x app_key_idx:%x\n", str, p_state->model_app.model_id, p_state->model_app.app_key_index);
        break;
        */
    case WICED_BT_MESH_CORE_STATE_IV:
        WICED_BT_TRACE("%sIV index:%x update:%x\n", str, p_state->iv.index, p_state->iv.update_flag);
        mesh_application_iv(p_state->iv.index, p_state->iv.update_flag);
        break;
    case WICED_BT_MESH_CORE_STATE_SUB_ADD:
        WICED_BT_TRACE("%sSUB_ADD cid:%04X model:%04X addr:%04X\n", str, p_state->model_sub_pub.company_id, p_state->model_sub_pub.model_id, p_state->model_sub_pub.sub_pub_addr);
        break;
    case WICED_BT_MESH_CORE_STATE_PUB_ADD:
        if((mesh_app_provision_status == MESH_APP_PROVISION_STATUS_PROVISIONING) || (mesh_app_provision_status == MESH_APP_PROVISION_STATUS_PROVISIONED))
        {
            wiced_bt_mesh_app_set_configured();
        }
        WICED_BT_TRACE("%sPUB_ADD cid:%04X model:%04X addr:%04X\n", str, p_state->model_sub_pub.company_id, p_state->model_sub_pub.model_id, p_state->model_sub_pub.sub_pub_addr);
        break;
    default:
    	break;
    }
...
wiced_bool_t mesh_application_seq_init(void)
{
    ...
    //LOG_DEBUG("mesh_application_seq_init:node_authenticated:%d\n", node_authenticated);
...
static char nible_to_char(uint8_t val)
{
    if (val < 10)
    {
        return '0' + val;
...
char *uuid_to_str(const uint8_t* uuid)
{
    static char str[3 * MESH_DEVICE_UUID_LEN];
    int i;
    int index = 0;
    for (i = 0; i < MESH_DEVICE_UUID_LEN; i++)
    {
        str[index++] = nible_to_char(uuid[i] >> 4);
        str[index++] = nible_to_char(uuid[i] & 0xF);
        str[index++] = ':';
    }
    str[index - 1] = '\0';
    return str;
...
typedef struct DYNAMIC_MEMORY_POOL
{
    ...
    BYTE*   first_free;
    UINT8   num_free;
    UINT8   die_reserve_count;
    void*   notify_first;
    void*   notify_last;
    UINT16  low_water_mark;
    UINT8   pool_name[4];
...
extern void event_alloc_print(void);
void deb_print_buf_use()
{
    ...
    WICED_BT_TRACE("fw size/count/free/low  %d/%d/%d/%d %d/%d/%d/%d %d/%d/%d/%d %d/%d/%d/%d\n",
                   g_dynamic_memory_GeneralUsePools[0].block_size,
                   g_dynamic_memory_GeneralUsePools[0].block_count,
                   g_dynamic_memory_GeneralUsePools[0].num_free,
                   g_dynamic_memory_GeneralUsePools[0].low_water_mark,
                   g_dynamic_memory_GeneralUsePools[1].block_size,
                   g_dynamic_memory_GeneralUsePools[1].block_count,
                   g_dynamic_memory_GeneralUsePools[1].num_free,
                   g_dynamic_memory_GeneralUsePools[1].low_water_mark,
                   g_dynamic_memory_GeneralUsePools[2].block_size,
                   g_dynamic_memory_GeneralUsePools[2].block_count,
                   g_dynamic_memory_GeneralUsePools[2].num_free,
                   g_dynamic_memory_GeneralUsePools[2].low_water_mark,
                   g_dynamic_memory_GeneralUsePools[3].block_size,
                   g_dynamic_memory_GeneralUsePools[3].block_count,
                   g_dynamic_memory_GeneralUsePools[3].num_free,
                   g_dynamic_memory_GeneralUsePools[3].low_water_mark);
...
#define MESH_APP_TIMER_CB_NUM  4
static mesh_app_timer_cb_t mesh_app_timer_cb[MESH_APP_TIMER_CB_NUM] = { 0 };

void wiced_bt_mesh_app_add_app_timer_cb(mesh_app_timer_cb_t app_timer_cb)
{
    int i;

    // First check whetehr the timer has been enabled already
    for (i = 0; i < MESH_APP_TIMER_CB_NUM; i++)
    {
        if (mesh_app_timer_cb[i] == app_timer_cb)
        {
            return;
        }
    }
    ...
    for (i = 0; i < MESH_APP_TIMER_CB_NUM; i++)
    {
        if (mesh_app_timer_cb[i] == NULL)
        {
            mesh_app_timer_cb[i] = app_timer_cb;
            return;
        }
    }
}

void wiced_bt_mesh_app_remove_app_timer_cb(mesh_app_timer_cb_t app_timer_cb)
{
    int i;
    for (i = 0; i < MESH_APP_TIMER_CB_NUM; i++)
    {
...
void wiced_bt_mesh_app_get_iv(uint32_t *p_iv_index, uint8_t *p_iv_update_flag)
{
    uint8_t data[8];
    uint8_t *p_data = data;
    wiced_result_t result;

    mesh_nvram_access(WICED_FALSE, NVRAM_ID_LOCAL_IV, data, 8, &result);
    if (result == WICED_BT_SUCCESS)
    {
        STREAM_TO_UINT32(*p_iv_index, p_data);
        STREAM_TO_UINT8(*p_iv_update_flag, p_data);
    }
    else
    {
        *p_iv_index = 0;
...
void wiced_bt_mesh_app_del_seq(uint16_t addr)
{
    // TBD" delete RPL entry
    wiced_bt_mesh_core_del_seq(addr);
}


uint8_t wiced_bt_mesh_app_is_provisioner(void)
{
    uint8_t is_provisioner;
    wiced_result_t result;
    mesh_nvram_access(WICED_FALSE, NVRAM_ID_LOCAL_PROVISIONER, &is_provisioner, sizeof(uint8_t), &result);
    if (result == WICED_BT_SUCCESS)
    {
...
// void wiced_bt_mesh_app_set_provisioner(void)
// {
//     wiced_result_t result;
//     uint8_t is_provisioner;
...
//     {
//         is_provisioner = 1;
//         mesh_nvram_access(WICED_TRUE, NVRAM_ID_LOCAL_PROVISIONER, &is_provisioner, sizeof(uint8_t), &result);
//     }
// }


static uint8_t mesh_app_config_timeout = 0;


static void mesh_app_config_timer(void)
{
    LOG_DEBUG("config_tmr: status:%d %d\n", mesh_app_provision_status, mesh_app_config_timeout);
    if (mesh_app_config_timeout > 0)
    {
        mesh_app_config_timeout--;
        return;
    }
    // Configuration timeout indicates configuration failure. Do factory reset to allow
    // the node to be provisioned again
    mesh_application_factory_reset();
}
...
static void mesh_application_iv(uint32_t iv_index, uint8_t iv_update_flag)
{
    ...
    UINT32_TO_STREAM(p_data, iv_index);
    UINT8_TO_STREAM(p_data, iv_update_flag);
    mesh_nvram_access(WICED_TRUE, NVRAM_ID_LOCAL_IV, data, (uint16_t)(p_data - data), &result);
}

static void mesh_application_provision_server_callback(uint16_t event, void *p_data)
{
    wiced_result_t result;
    const char* str = "prov_cb: ";
    switch (event) {
    case WICED_BT_MESH_PROVISION_STARTED:
        LOG_DEBUG("%sstarted\n", str);
        mesh_app_provision_status = MESH_APP_PROVISION_STATUS_PROVISIONING;
        // Now store the provisioing flag into NVRAM in case the device is powered off in the middle of provisioing/configuring process
        mesh_nvram_access(WICED_TRUE, NVRAM_ID_LOCAL_PROVISION, &mesh_app_provision_status, sizeof(uint8_t), &result);
        break;
        /*
    case WICED_BT_MESH_PROVISION_GET_OOB_DATA:
        WICED_BT_TRACE("%soob\n", str);
        ...
        */
    case WICED_BT_MESH_PROVISION_END:
        LOG_DEBUG("%sprovisioned\n", str);

        // Node provisioned with a unicast address. Set a timer for configuration timeout.
        // Occasionally we may receive the app_key_added event before this provision_end
        // event. In that case, the status flag will be set to as configured and the NVRAM
        // record is cleared. Hence we shall make sure the status is provisioning first
        // before we enable the timer to avoid out of sync.
        if (mesh_app_provision_status == MESH_APP_PROVISION_STATUS_PROVISIONING)
        {
            mesh_app_provision_status = MESH_APP_PROVISION_STATUS_PROVISIONED;
            wiced_bt_mesh_app_add_app_timer_cb(mesh_app_config_timer);
            mesh_app_config_timeout = MESH_APP_CONFIG_TIMEOUT;
        }
        break;
    default:
        break;
    }
	mesh_app_provision_handler( event,  p_data);
}

void wiced_bt_mesh_app_set_configured(void)
{
    wiced_result_t result;

    WICED_BT_TRACE("app_set_configured\n");

    // The device configuration is done. We can clear the configuration timer now.
    wiced_bt_mesh_app_remove_app_timer_cb(mesh_app_config_timer);
    mesh_app_provision_status = MESH_APP_PROVISION_STATUS_CONFIGURED;

    // Delete the NVRAM record for provisioing value
    mesh_nvram_access(WICED_TRUE, NVRAM_ID_LOCAL_PROVISION, NULL, 0, &result);

    // If it is provisoner, mesh_state_changed_cb() will not be called with
    // WICED_BT_MESH_CORE_STATE_NODE_STATE, hence manually set here.
    if (wiced_bt_mesh_app_is_provisioner())
    {
        WICED_BT_TRACE("app_set_configured: provisioner\n");
        node_authenticated = WICED_TRUE;
        mesh_app_gatt_db_init(node_authenticated);
    }
}
#if defined HCI_CONTROL
void mesh_app_raw_access_layer_mode_send_tx_complete(uint16_t hci_opcode)
{
    wiced_bt_mesh_hci_event_t* p_hci_event;
    wiced_bt_mesh_event_t event = { 0 };
    event.hci_opcode = hci_opcode;
    event.status.tx_flag = TX_STATUS_COMPLETED;
    if ((p_hci_event = wiced_bt_mesh_create_hci_event(&event)) != NULL)
        wiced_bt_mesh_send_hci_tx_complete(p_hci_event, &event);
}

void mesh_app_set_raw_access_layer_mode_handler(wiced_bt_mesh_core_received_msg_handler_t msg_handler)
{
    WICED_BT_TRACE("set_raw_access_layer_mode_handler: msg_handler:%x\n", msg_handler);
    mesh_app_raw_access_layer_mode_msg_handler_ptr = msg_handler;
    mesh_app_raw_access_layer_mode_send_tx_complete(HCI_CONTROL_MESH_COMMAND_SET_RAW_ACCESS_LAYER_MODE);
}

void mesh_app_cancel_raw_access_layer_pdu()
{
    WICED_BT_TRACE("cancel_raw_access_layer_pdu: p_event=%x\n", mesh_app_send_raw_access_layer_pdu_p_event);
    // cancel outstending wiced_bt_mesh_core_send(), delete remembered p_event and remember this p_event
    // we allow only one outstending wiced_bt_mesh_core_send() in raw_access_layer mode
    if (mesh_app_send_raw_access_layer_pdu_p_event)
    {
        wiced_bt_mesh_core_cancel_send(mesh_app_send_raw_access_layer_pdu_p_event);
        wiced_bt_mesh_release_event(mesh_app_send_raw_access_layer_pdu_p_event);
        mesh_app_send_raw_access_layer_pdu_p_event = NULL;
    }
}

const uint8_t* mesh_app_raw_access_layer_dev_key_callback(uint16_t addr, uint16_t* p_net_key_idx)
{
    WICED_BT_TRACE("raw_access_layer_dev_key_callback: addr:%x clnt_addr:%x\n", addr, mesh_app_raw_access_layer_clnt_addr);
    if (addr != mesh_app_raw_access_layer_clnt_addr)
        return NULL;
    if (p_net_key_idx)
        *p_net_key_idx = mesh_app_raw_access_layer_clnt_netkey_idx;
    return mesh_app_raw_access_layer_clnt_devkey;
}
void mesh_app_set_raw_access_layer_clnt_devkey(uint16_t addr, uint16_t netkey_idx, uint8_t* devkey)
{
    WICED_BT_TRACE("set_raw_access_layer_clnt_devkey: addr:%x netkey_idx:%x devkey:%02x...%02x\n", addr, netkey_idx, devkey[0], devkey[15]);
    mesh_app_cancel_raw_access_layer_pdu();
    wiced_bt_mesh_core_set_dev_key_callback(mesh_app_raw_access_layer_dev_key_callback);
    mesh_app_raw_access_layer_clnt_addr = addr;
    mesh_app_raw_access_layer_clnt_netkey_idx = netkey_idx;
    memcpy(mesh_app_raw_access_layer_clnt_devkey, devkey, 16);
    mesh_app_raw_access_layer_mode_send_tx_complete(HCI_CONTROL_MESH_COMMAND_SET_RAW_ACCESS_LAYER_CLNT_DEVKEY);
}

void mesh_app_send_raw_access_layer_pdu_cb(wiced_bt_mesh_event_t* p_event)
{
    wiced_bt_mesh_hci_event_t* p_hci_event;
    WICED_BT_TRACE("send_raw_access_layer_pdu_cb: p_event:%x\n", p_event);
    p_event->hci_opcode = HCI_CONTROL_MESH_COMMAND_SEND_RAW_ACCESS_LAYER_PDU;
    if ((p_hci_event = wiced_bt_mesh_create_hci_event(p_event)) != NULL)
        wiced_bt_mesh_send_hci_tx_complete(p_hci_event, p_event);
    wiced_bt_mesh_release_event(p_event);
    // reset remembered p_event - we allow only one outstending wiced_bt_mesh_core_send() in raw_access_layer mode
    mesh_app_send_raw_access_layer_pdu_p_event = NULL;
}
...
{
    wiced_bt_mesh_event_t               *p_event;
    WICED_BT_TRACE("send_raw_access_layer_pdu: ttl:%x element_idx:%x addr:%x app_key_idx:%x company_id:%x model_id:%x opcode:%x cnt:%d interval:%d params_len:%d\n",
        ttl, element_idx, addr, app_key_idx, company_id, model_id, opcode, retrans_cnt, retrans_interval, params_len);
    mesh_app_cancel_raw_access_layer_pdu();
    if (NULL != (p_event = wiced_bt_mesh_create_event(element_idx, company_id, model_id, addr, app_key_idx)))
    {
//        if(flags & TAG_USE_DIRECTED)
//            p_event->flags = TAG_USE_DIRECTED;
        p_event->opcode = opcode;
        p_event->ttl = ttl;
        p_event->retrans_cnt = retrans_cnt;
        p_event->retrans_time = retrans_interval;
        mesh_app_send_raw_access_layer_pdu_p_event = p_event;
        wiced_bt_mesh_core_send(p_event, params, (uint16_t)params_len, mesh_app_send_raw_access_layer_pdu_cb);
    }
}
...
wiced_bool_t mesh_app_raw_access_layer_mode_msg_handler(wiced_bt_mesh_event_t* p_event, const uint8_t* params, uint16_t params_len)
{
    ...
    //WICED_BT_TRACE("raw_access_layer_mode_msg_handler\n");
    if (mesh_app_raw_access_layer_mode_msg_handler_ptr)
        mesh_app_raw_access_layer_mode_msg_handler_ptr(p_event, params, params_len);
    // If app didn't return message handler in the latest call to get_msg_handler_callback() then return FALSE meaning p_event isn't released.
    // Otherwise call that message handler returning it's result and reset app message handle
    if (mesh_app_raw_access_layer_mode_app_handler_ptr)
        res = mesh_app_raw_access_layer_mode_app_handler_ptr(p_event, params, params_len);
    mesh_app_raw_access_layer_mode_app_handler_ptr = NULL;
    return res;
}
#endif
